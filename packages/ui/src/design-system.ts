/**
 * the published component set — these 40 names, and only these, are what
 * `.design-sync/config.json` → `componentSrcMap` lists today. it lives in its
 * own module so the set stays greppable: a component the design agent can't
 * see is a component the next screen invents from scratch instead, and an
 * export that drifts in here silently becomes something it designs with.
 *
