...
interface StepsCarouselProps {
  classes?: string;
  dot_position?: "top" | "bottom";
  hand_position?: "top-left" | "right";
}

export function StepsCarousel({
  classes = "",
  dot_position = "top",
  hand_position = "top-left",
}: StepsCarouselProps): React.JSX.Element {
  const container_ref = useRef<HTMLDivElement>(null);
  use_drag_scroll(container_ref);
  ...
  const scroll_to = (i: number) => {
    const child = container_ref.current?.children[i];
  ...
  const dot_classes =
    dot_position === "top"
      ? "absolute top-1 left-0 right-0 flex justify-center"
      : "absolute -bottom-2 left-0 right-0 flex justify-center";

  const hand_classes =
    hand_position === "top-left"
      ? "absolute grid -top-8 left-4 z-10"
      : "max-md:hidden absolute grid top-3/4 -right-20 z-10";

  return (
    <div className={`${classes} relative pt-8`}>
      <div
        ref={container_ref}
        className="flex gap-4 overflow-x-auto snap-x snap-mandatory scroll-smooth scrollbar-none cursor-grab"
      >
        {screenshots.map((screenshot, i) => (
          <div className="flex-[0_0_100%] snap-start" key={i}>
            <Image
              src={screenshot}
      ...
      <div className={hand_classes}>
        ...
        <p className="font-gochi max-w-[70%] text-center text-lg font-medium">
          Do&nbsp;Swipe!
        </p>
      </div>
    </div>
