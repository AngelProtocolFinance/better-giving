        ...
        <Tabs.List className="flex gap-2 border-b">
          {tabs.map((tab) => (
            <Tabs.Trigger
              key={tab.id}
              value={tab.id}
              className="px-4 border-b-2 py-2 text-sm font-medium transition-colors focus:outline-none border-transparent text-gray-11 hover:text-gray-12 data-selected:border-primary data-selected:text-primary"
            >
              {tab.name}
            </Tabs.Trigger>
          ))}
