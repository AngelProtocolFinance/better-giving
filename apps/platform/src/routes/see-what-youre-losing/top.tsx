      ...
      <div>
        <h1 className="text-4.5xl leading-snug max-lg:text-center text-right">
          <div className="relative w-fit max-lg:mx-auto lg:ml-auto text-nowrap">
            ...
            <span className="absolute bottom-2 left-0 w-full h-4 bg-secondary -z-10" />
          </div>
        </h1>
        <p className="mb-4 text-lg max-lg:text-center text-right mt-4">
          Thousands of dollars are slipping through the cracks every year – lost
          to fees, idle cash, and outdated donation methods.
        </p>
        <p className="mb-4 font-semibold text-lg max-lg:text-center text-right mt-4">
          The longer you wait, the more you miss.
        </p>
        <div className="mb-4 text-lg max-lg:text-center text-right">
        ...
        <button
          onClick={() => {
            if (state !== "loaded") {
              return navigate(href("/donation-calculator"));
            }
            set_open(true);
          }}
          type="button"
          className="mt-8 capitalize btn btn-lg btn-primary text-center lg:text-right justify-self-center lg:justify-self-end w-full md:w-auto"
        >
...
function Tooltip({ className = "" }) {
  return (
    <span className={className}>
      ...
      <p className="-top-6 left-8 relative -rotate-12 font-gochi text-nowrap">
        Watch how it works!
      </p>
    </span>
  );
