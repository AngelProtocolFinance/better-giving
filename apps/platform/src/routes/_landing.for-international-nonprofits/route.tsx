...
export default function Page() {
  ...
  return (
    <main>
      <Hero
        classes="border-b border-border px-5 md:px-6 pt-9 md:pt-14 pb-9 md:pb-12"
        errors={result?.errors}
        values={result?.values}
        signed_in_as={result?.signed_in_as}
        pending={nav.state === "submitting"}
      />
      <Credentials classes="bg-card px-5 md:px-6 py-4" />
      <TrustBar
        classes="bg-muted border-y border-border px-5 md:px-6 py-7"
        items={stats}
      />
      ...
      <CtaBand
        ...
        subtitle="One short application, reviewed in 2-3 days, is all that stands between your organization and tax-deductible U.S. fundraising."
