...
- **page shape is the exception, and it runs the other way.** `page` in `packages/ui/src/styles/utilities.css` is the single page shape: it owns its own max-width, inline centering and gutter (`px-5 md:px-10`), and a caller must not re-specify any of the three. two utilities of equal specificity resolve by stylesheet source order, not class-string order, so a leftover `px-6`/`mx-auto`/`max-w-*` beside it is a coin flip that renders right on some sites and wrong on others. its width curve is tailwind's own `container` ladder spelled out — full-bleed, then 80rem, then 96rem — so every page widens with the window exactly as it did when each site hand-spelled `xl:container xl:mx-auto px-*`; what the name replaces is the gutter spread (px-5/px-10/px-24) that kept a header and the sections under it from sharing an edge. **reading measure is not a page width** — cap the text column one level in (`max-w-3xl` on the paragraph block, or `prose`, which carries its own 65ch), never by narrowing the page around it. the full-bleed band that paints a section's background is a separate element wrapping `page` and carries the fill and the vertical rhythm only — never a horizontal padding, which would stack a second gutter; a page that alternates full-bleed bands with contained ones therefore puts `page` on each band, never hoisted onto a shared parent. hand-spelling `xl:container` is what the sweep test fails on, not the utility's behaviour.
- **a wide table scrolls, it does not widen the page.** `table-scroll` in `packages/ui/src/styles/utilities.css` goes on the element wrapping a `<table>`, never on the table — a table's columns are the data and cannot reflow, so past a narrow viewport it either scrolls sideways or pushes the page (and the dashboard sidebar with it) wider. it is `overflow-x-auto` plus `scrollbars`, so writing both beside it is the hand-spelling the name replaced. `scrollbars` on its own is the thin themed skin for anything else that scrolls — a popup list, a code block, a drawer; the three classes behind it were always written as one breath, and half-spelling them is how a scroller ends up thin-but-uncoloured. structure (a `grid`, a `col-span-full`, a fill, a margin) stays the caller's and does not compete.
- **a card is `card`, and its inset is 24px everywhere.** `card` in `packages/ui/src/styles/utilities.css` is `bg-panel border rounded p-6` — the fill, the edge, the corner and the inset, and nothing else. it replaced 30 hand-spelled insets across the tree, so writing those four classes out is what `shell-conformance.node.test.ts` fails on, as is a second fill/edge/corner/padding beside the name. layout, gap, type and outer margin stay the caller's and sit beside it; a marketing card that lifts writes `card shadow-lift-card`. a surface with **no** inset has delegated it and is a different shell (table shell, `divide-y` list, `overflow-hidden` media frame) — leave those alone, and likewise a floating layer, a control's box and a pinned band.
- **a text link is `link`, and its hover is a colour shift.** `link` in `packages/ui/src/styles/utilities.css` is ink (`--primary`), hover ink (`--primary-hover`) and the focus ring — nothing else, so size, weight, spacing and display stay the caller's and a resting `underline` beside it is fine. it replaced six spellings across 139 sites, so writing a `hover:text-*` or a `hover:underline` on an `<a>`/`Link`/`NavLink`/`ExtLink` is what `link-conformance.node.test.ts` fails on. **three shapes are not this and the sweep leaves them alone**: a link that paints a surface of its own (a menu row, a tile, a tab — its ink moves with the fill), a link on a coloured ground (the footer, the announcement banner and the one `bg-primary` pillar keep the contrast ink; a destructive row action keeps the error ink), and a nav marking its current item by ink (the dashboard sidebar, the marketing header). a link on a brand fill has **no** hover rung — the `--primary-fg` alpha ladder is unmeasured and must not become a scale by accretion, so those keep what they have.
- **the three sidebar surfaces share one shell.** `Layout` in `src/layout/dashboard/layout.tsx` is the whole dashboard page — sticky app header, sidebar/views grid, minimal footer — and donor (`routes/dashboard`), npo admin (`routes/admin.$id`) and platform admin (`routes/platform`) each render only it. they differ in links, sidebar header, and `headerVariant` (the donor dashboard drops the header's auth slot). the route module keeps its own `middleware`, `loader` and `meta`: those are route-module contracts and cannot be lifted into a component. a route module reaching for `AppHeader` or `Footer` beside `Layout` is what the sweep fails on — the three had already drifted apart on the footer's margin before this collapsed.
