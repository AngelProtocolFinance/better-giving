...
// define only replaces that exact spelling. `process.env[name]` would match
// only the `process.env: {}` catch-all beside them, so every key would read
// undefined and every `required` call would throw.
