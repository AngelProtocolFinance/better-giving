...
- workspace members declared in `pnpm-workspace.yaml` (`packages: [apps/*, packages/crypto, packages/emails, packages/paypal, packages/stocks, packages/types/*]`). Add a member there + give it a `package.json` and `tsconfig.json` that extends the base (built members included — they just add emit config on top).
