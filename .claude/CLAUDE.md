...
- **`packages/brand/`** — internal package `@better-giving/brand` (private): the canonical Better Giving social profile urls (`socials`), a single `src/index.ts` with zero deps. **Source-only** (`exports: "./src/index.ts"`, no build, no `dist/`) — consumers transpile it. Shared by platform (spread into its wider `socials` map in `src/constants/urls.ts`) and the docs footer. Its whole surface is one const; no `CLAUDE.md`.
