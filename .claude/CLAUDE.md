...
- `pnpm test` at root runs platform's and ui's browser-mode suites concurrently under turbo — two chromiums at once (the `strictPort` comment in each `vite.config.ts` exists for that collision). Scope to one member: `pnpm --filter <pkg> test`.
