...
- **Smoke check** — `.github/workflows/smoke.yml` runs on Vercel's `vercel.deployment.ready` dispatch for every platform deployment (docs and emails-preview skip) and requests `/`, `/api/currencies` and `/form-embed.js` on the deployment's own URL, so a server that crashes at boot fails before promotion — `checks`/`test` never run the built server. It reports as the **commit status** `smoke (<environment>)`; the one to require is `smoke (production)`, not the `smoke-dispatch` job, whose check run lands on `main`'s HEAD. Bot Protection challenges every non-browser request, so the run sends an `x-smoke-key` header from the `SMOKE_BYPASS_KEY` repo secret, which the platform project's WAF custom rule `smoke check bypass` matches with action `bypass` — first in rule order, and scoped to `*.vercel.app` hosts and the three paths, so better.giving never gets the bypass and a path added to `smoke.yml` needs adding to the rule. Key and secret must match — rotating one means rotating both; on a mismatch every run fails closed.
