...
export function Team({ classes = "" }) {
  return (
    <div className={`${classes} grid pb-20`}>
      <h2 className="mx-auto text-3xl/tight md:text-4xl/tight lg:text-4.5xl/tight max-md:text-center text-balance mb-16 mt-4 text-primary">
  ...
  return (
    <div className="grid md:grid-cols-[auto_1fr] xl:grid-rows-subgrid xl:row-span-2 gap-4 max-md:justify-items-center">
      <div className="space-y-4 self-start row-span-2 relative">
        <img
          width={150}
          className="-mt-1 rounded-full bg-muted-fg object-contain shadow-2xl aspect-square md:row-span-3"
          src={props.image}
          alt=""
        />
        <div className="absolute -translate-x-1/2 left-1/2 flex items-center justify-center gap-2">
          {props.socials.github && (
            <ExtLink href={props.socials.github} className="text-primary">
              <img width={21} src={github} alt="" />
            </ExtLink>
          )}
          {props.socials.linkedin && (
            <a href={props.socials.linkedin} className="text-primary">
              <img width={20.5} src={linkedin} alt="" />
            </a>
          )}{" "}
          {props.socials.email && (
            <ExtLink href={props.socials.email} className="text-primary">
              <Mail size={22} className="stroke-primary" />
            </ExtLink>
          )}
        </div>
      </div>
      <div className="self-end w-full max-md:mt-4">
        <p className="text-primary max-md:mt-2 uppercase font-bold text-lg max-md:text-center">
          {props.name}
        </p>
        ...
        {more.length > 0 ? (
          <>
            {expanded
              ...
              : null}
            <button
              className="text-muted-fg  hover:text-fg mt-2 block max-md:mx-auto"
              type="button"
