...
export default function SearchDropdown({ classes = "", page1, query }: Props) {
  ...
  return (
    <div
      className={`${classes} bg-popover text-popover-fg container rounded p-6 shadow-2xl shadow-black/20`}
    >
      {state === "idle" && !query ? (
        <>
    ...
    <div className="flex flex-wrap items-center gap-4">
      {props.items.map((endow) => (
        ...
        >
          ...
          <span className="text-muted-fg  group-hover/item:text-primary">
            {endow.name}
          </span>
        </Link>
      ))}
      {props.page < props.pages && (
        <Link
          className="w-full text-primary font-medium text-lg text-center mt-8 block"
          to={{
            pathname: href("/marketplace"),
            search: new URLSearchParams({
              query: props.query,
