...
import type { Payload as BankingPayload } from "@/queue/msgs/banking-approved";
...
export async function handle_banking_new_account(data: BankingPayload) {
...
export async function handle_banking_approved(data: BankingPayload) {
  await send_banking_email(data.npo_id, {
    action: "approved",
    account_summary: data.bank_summary ?? "",
  });
...
export async function handle_banking_rejected(data: BankingPayload) {
  await send_banking_email(data.npo_id, {
    action: "rejected",
    account_summary: data.bank_summary ?? "",
...
export async function handle_banking_set_default(data: BankingPayload) {
  await send_banking_email(data.npo_id, {
    action: "default",
    account_summary: data.bank_summary ?? "",
  });
