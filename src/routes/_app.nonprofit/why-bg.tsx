...
export default function WhyBG({ className = "" }) {
  return (
    <section className={`${className} grid`}>
      ...
      <p className="text-center mb-16  col-span-full text-xl mt-4">
        A partner who understands your needs.
      </p>

      <ul className="mt-4 @3xl:divide-x divide-border grid @3xl:grid-cols-3 gap-y-16 @3xl:gap-y-0">
        {items.map((item, idx) => (
      ...
      <div className="isolate z-10 flex flex-col @3xl:flex-row items-center justify-self-center gap-4 mt-24">
        <Link
          ...
          className="btn-primary isolate inline-flex items-center px-10 py-3 gap-1 rounded text-lg  relative group active:translate-x-1 font-bold shadow-2xl"
        >
        ...
        <Link
          to={BOOK_A_DEMO}
          className="isolate inline-flex items-center px-10 py-3 gap-1 rounded text-lg  relative bg-card group active:translate-x-1 text-primary font-bold shadow-2xl shadow-black/10 border"
        >
  ...
  return (
    <li className="grid content-start @3xl:grid-rows-subgrid row-span-4 justify-items-center px-4">
      <Image
        ...
        className="mb-4"
      />
      <h5 className="text-center  font-medium text-2xl">{props.title1}</h5>
      <h6 className="text-center self-center font-normal  text-lg @6xl:text-xl text-muted-fg mb-4">
        {props.title2}
      </h6>
      <p className="pt-4 text-center @6xl:text-lg text-muted-fg border-t-[3px] border-primary ">
        {props.description}
      </p>
    </li>
  );
