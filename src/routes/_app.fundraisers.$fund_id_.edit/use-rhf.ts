...
export function use_rhf(init: IFund) {
  const {
    ...
    values: {
      name: init.name,
      description: to_content(init.description_rich ?? undefined),
      slug: init.slug ?? "",
      target:
        init.target === "0"
          ? { type: "none" }
  ...
  // preserve FV generic — inference widens to FieldValues due to `control as any` in useFieldArray
  const handleSubmit: UseFormHandleSubmit<FV> = _handleSubmit;
