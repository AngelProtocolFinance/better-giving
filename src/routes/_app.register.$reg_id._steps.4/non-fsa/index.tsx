...
import type { INpoWithRegNum } from "@/npo";
    ...
    if (fv.o_ein !== props.ein) {
      ...
      if (endow) {
        if (endow.claimed ?? true) {
          return show_toast({
            type: "info",
            message: `Nonprofit: ${endow.name} with EIN: ${fv.o_ein} already exists on our app. You must speak with an existing user of your NPO Account's members in order to be invited on as a member.`,
          });
        }
